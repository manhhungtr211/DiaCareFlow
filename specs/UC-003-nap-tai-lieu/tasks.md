...
- [X] T009 [P] [US1] Implement embedding module tại `src/ingestion/embedding.py` — hàm `embed_chunks(chunks: list[DocumentChunk]) -> list[EmbeddedChunk]` sử dụng "keepitreal/vietnamese-sbert" để tạo vector cho từng chunk. Mỗi EmbeddedChunk có id, vector, payload (content, source, page, chunk_index, document_id)
