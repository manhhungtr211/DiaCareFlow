...
4. Không bịa đặt bất kỳ thông tin nào. Chỉ sử dụng những gì có trong lịch sử trò chuyện hoặc ngữ cảnh được trích xuất.
...
def response_agent_node(state: AgentState) -> dict[str, Any]:
    ...
    # Safely log input context without dumping the whole state object
    logger.info(
        f"Response Agent Input: user_input='{state.get('user_input', '')}', "
        f"is_safe={state.get('is_safe', True)}, "
        f"follow_up_question='{state.get('follow_up_question', '')}', "
        f"should_response={state.get('should_response', False)}, "
        f"factor_results_count={len(state.get('factor_results', []))}, "
        f"suggestion_results_count={len(state.get('suggestion_results', []))}, "
        f"harm_results_count={len(state.get('harm_results', []))}"
    )
    ...
    try:
        ...
        if not is_safe:
            ...
            logger.info(f"Response Agent Output: {output}")
            return output

        # --- FOLLOW UP path: return the question directly ---
        if follow_up_question:
            logger.info("Response Agent: returning follow_up_question")
            output = {
                "response_context": {
                    "final_answer": follow_up_question,
                    "sources": [],
                    "is_refused": False,
                    "refuse_reason": None,
                },
        ...
        if suggestion_summary:
            context_parts.append(f"**Giải pháp / Lời khuyên:**\n{suggestion_summary}")
        if harm_summary:
            context_parts.append(f"**Tác hại / Cảnh báo:**\n{harm_summary}")

        if not context_parts and not should_response:
            # All sub-agents failed, and it wasn't a small talk / direct response query
...
Câu trả lời tổng hợp:"""
        ...
        llm = ChatGroq(model_name=ROUTING_MODEL, temperature=0.4)
        ...
        logger.info(f"Response Agent Output: {result}")
        return result
